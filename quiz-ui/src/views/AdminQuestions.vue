<template>
	<div class="questions-module">
		<div ref="questionsContainer" class="questions-container">
			<div v-for="(question, qIndex) in questions" :key="question.id" :class="[
				'question-card',
				{ 'new-question': question.status === 'new', 'edited-question': question.status === 'edited' },
			]">
				<div class="question-image">
					<img :src="question.image" alt="Question Image" />
				</div>
				<div class="question-content">
					<input type="text" v-model="question.title" placeholder="Title" @input="markEdited(qIndex)" />
					<textarea v-model="question.text" placeholder="Question text"
						@input="markEdited(qIndex)"></textarea>
					<div v-for="(answer, aIndex) in question.possibleAnswers" :key="answer.id" class="answer">
						...
						<input type="checkbox" v-model="answer.isCorrect"
							@change="ensureSingleCorrectAnswer(qIndex, aIndex); markEdited(qIndex)" />
						Correct
				...
				<div class="question-buttons">
					<button @click="confirmDelete(qIndex)" class="delete-button">Delete</button>
					<button v-if="question.status !== 'current'" @click="updateQuestion(qIndex)">
		...
		<delete-modal v-if="showModal" @close="closeModal" @confirm="deleteQuestion" />
	</div>
</template>
...
export default {
	...
	methods: {
		...
		addAnswer(qIndex) {
			...
			this.questions[qIndex].possibleAnswers.push({ id: newAnswerId, text: '', isCorrect: false });
		...
		async updateQuestion(index) {
			...
			question.possibleAnswers = question.possibleAnswers.filter((answer) => answer.text.trim() !== '');
